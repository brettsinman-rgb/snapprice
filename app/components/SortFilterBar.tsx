...
export default function SortFilterBar({
  ...
  return (
    <div className="flex min-w-0 flex-wrap items-center gap-2 rounded-2xl border border-[#0FF7D0]/25 bg-[#202020] p-3 shadow-[0_20px_60px_-42px_rgba(0,0,0,0.9)] sm:gap-3 sm:p-4">
      <div className="inline-flex shrink-0 overflow-hidden rounded-full border border-white/10 bg-white/[0.03] p-1">
        {[
          ...
          <button
            ...
            className={clsx(
              ...
              sortMode === option.id
                ? 'border-transparent bg-[#0FF7D0] text-[#111111] shadow-sm'
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-2 sm:gap-3">
        ...
        <select
          ...
          className="select-cta select-cta-pill h-10 max-w-full rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-white hover:border-[#0FF7D0]/35 sm:px-5 sm:tracking-[0.18em]"
        >
          ...
          <div ref={regionMenuRef} className="relative">
            ...
            {regionsOpen && (
              <div
                ...
                className="fixed left-4 right-4 top-[var(--region-menu-top)] z-50 max-h-[calc(100vh-var(--region-menu-top)-16px)] w-auto overflow-y-auto rounded-2xl border border-[#0FF7D0]/15 bg-[#252525] p-3 text-xs text-white shadow-[0_22px_70px_-38px_rgba(0,0,0,0.95)] sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-2 sm:max-h-none sm:w-72 sm:max-w-[calc(100vw-2rem)] sm:overflow-visible"
              >
                ...
                <div className="mt-2 grid gap-1">
                  {regionOptions.map((region) => {
                    ...
                    return (
                      <label
                        key={region.id}
                        className="flex cursor-pointer items-center gap-3 rounded-xl px-2 py-2 text-sm font-medium text-white hover:bg-[#0FF7D0]/10"
                      >
                        <input
                          type="checkbox"
                          checked={checked}
                          ...
                          className="h-4 w-4 rounded border-[#0FF7D0]/40 bg-[#202020] text-[#0FF7D0] focus:ring-[#0FF7D0]"
                        />
